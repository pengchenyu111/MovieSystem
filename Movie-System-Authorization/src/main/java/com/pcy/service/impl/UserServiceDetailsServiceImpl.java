...
import com.pcy.mapper.UserAdminMapper;
...
public class UserServiceDetailsServiceImpl implements UserDetailsService {
    ...
    private String adjustUsername(String username, String loginType) {
        ...
        if (LoginConstant.ADMIN_TYPE.equals(loginType)) {
            return userAdminMapper.selectUsernameById(username);
        }
        // 普通用户
        if (LoginConstant.GENERAL_USER_TYPE.equals(loginType)) {
            return userGeneralMapper.selectUsernameById(username);
        }
        return username;
    }
    ...
    private UserDetails loadSysUserByUsername(String username) {
        UserAdmin userAdmin = userAdminMapper.selectSysUserByUsername(username);
        return new User(
                String.valueOf(userAdmin.getId()),
                userAdmin.getPassword(),
                // 是否可用
                userAdmin.getStatus() == 1,
                // 账号未过期
                true,
                // 凭证未过期
                true,
                ...
                getSysUserPermissions(userAdmin.getId())
        );
    }
    ...
    private UserDetails loadGeneralUserByUsername(String username) {
        UserGeneral generalUser = userGeneralMapper.selectGeneralUserByUsername(username);
        return new User(
